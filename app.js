...
createApp({
  ...
  <template v-if="role==='Customer'&&page==='Overview'">
    ...
    <div class="service-grid"><article class="service card" v-for="s in visibleServices" :key="s.id"><div class="service-icon">{{ t(s.icon) }}</div><small>{{ t(s.group) }}</small><h3>{{ t(s.name) }}</h3><p>{{ t(s.description) }}</p><div class="service-bottom"><div><b>{{ t(money(s.price)) }}</b><small>{{ t("per") }} {{ t(s.unit) }}</small></div><button class="round" :aria-label="t('Book '+s.name)" @click="selected=s;quantity=1;issue='';date=''">↗</button></div></article><p v-if="!visibleServices.length" class="card">{{ t("No published services here yet. Try another area.") }}</p></div>
